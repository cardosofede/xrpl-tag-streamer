...
class MongoDatabase:
    ...
    def initialize_default_users(self, default_users: Dict[str, List[str]]) -> None:
        """
        ...
        """
        for user_id, wallets in default_users.items():
            self.users.update_one(
                {"id": user_id},
                {"$set": {"id": user_id, "wallets": wallets}},
                upsert=True
            )
            logger.info(f"Initialized user {user_id} with {len(wallets)} wallets")
